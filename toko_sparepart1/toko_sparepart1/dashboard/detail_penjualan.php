...
$id = (int)($_GET['id'] ?? 0);
if (!$id) { header('Location: penjualan.php'); exit; }

// Ambil data transaksi
$stmt = $pdo->prepare("
    SELECT p.*, COALESCE(c.nama,'Umum') AS nama_pelanggan,
...
if (!$trx) { header('Location: penjualan.php'); exit; }
...
function metodeBadge($m){$map=['Tunai'=>'badge-blue','Transfer'=>'badge-green','QRIS'=>'badge-purple'];return $map[$m]??'badge-gray';}
function statusBadge($s){$map=['Selesai'=>'badge-success','Proses'=>'badge-blue','Dibatalkan'=>'badge-red'];return $map[$s]??'badge-gray';}
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<title>Detail Transaksi - Sparepart Store</title>
...
<div class="layout">
  ...
  <div class="main">
    <header class="topbar">
      <div class="topbar-left"><span class="hamburger">☰</span><h1>Detail Transaksi</h1></div>
    ...
    <div class="content">
      ...
      <div class="card" style="margin-bottom:20px;">
        <div class="card-header">
          ...
          <div style="display:flex;gap:8px;align-items:center;">
            <span class="badge <?= metodeBadge($trx['metode_pembayaran']) ?>"><?= htmlspecialchars($trx['metode_pembayaran']) ?></span>
        ...
        <div class="form-row" style="margin-top:16px;">
          <div class="form-group">
            <label style="color:#6b7280;font-size:12.5px;">Pelanggan</label>
            <p style="font-size:14px;color:#374151;margin:4px 0 0;"><?= htmlspecialchars($trx['nama_pelanggan']) ?></p>
          </div>
          <div class="form-group">
            <label style="color:#6b7280;font-size:12.5px;">Kasir</label>
            <p style="font-size:14px;color:#374151;margin:4px 0 0;"><?= htmlspecialchars($trx['nama_kasir']) ?></p>
          </div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="total-box" style="margin-top:20px;">
          <div class="total-row"><span>Subtotal</span><span><?= rp($trx['total'] + $trx['diskon']) ?></span></div>
          <?php if ($trx['diskon'] > 0): ?>
          <div class="total-row"><span>Diskon</span><span style="color:#dc2626;">- <?= rp($trx['diskon']) ?></span></div>
          <?php endif; ?>
          <div class="total-row grand"><span>TOTAL</span><span><?= rp($trx['total']) ?></span></div>
        </div>

        <!-- TOMBOL CETAK -->
        <div class="form-actions" style="margin-top:20px;">
          <a href="penjualan.php" class="btn-outline" style="padding:11px 20px;border-radius:9px;font-size:13.5px;font-weight:600;color:#374151;text-decoration:none;">← Kembali</a>
          <button onclick="window.print()" class="btn-primary" style="padding:11px 24px;">🖨️ Cetak Struk</button>
        </div>
      </div>
    </div>
  </div>
